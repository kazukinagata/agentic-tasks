...
```bash
# Write the full task page object to a temp file
echo '<task_json>' > /tmp/task_validate.json
bash ${CLAUDE_PLUGIN_ROOT}/skills/validating-fields/scripts/validate-task-fields.sh \
  "<target_status>" /tmp/task_validate.json
```

1. Fetch the full task page object and write it to a temp file
...
**Multi-round questioning**: For AC and Execution Plan, follow the brainstorming protocol. If the user's response lacks verifiable conditions (no commands, file paths, metrics, or observable outcomes), propose 3 concrete options and brainstorm together. If the user disengages, accept with `[LOW CONFIDENCE]` tag. See `planning-tasks` SKILL.md for the full protocol.

**Auto-planning shortcut**: If the user says "auto", "自動で", or "generate" for AC or Execution Plan, invoke the `planning-tasks` skill. If Description is too vague (no nouns, no context), ask the user to elaborate first.
