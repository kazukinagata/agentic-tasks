...
2. Create the main task:
   ...
   - Executor: Determine from message content — if the required action (after hearing) is clearly code/research/docs, infer executor (claude-code or cowork based on execution_environment). If unclear or requires human judgment → `human` (default, re-evaluated in daily routine Step 2.5 when unblocked).
