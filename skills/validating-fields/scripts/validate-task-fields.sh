...
#   task_json_file — Path to JSON file with a single Notion page object
#
# Output:
...
RESULT=$(jq --arg target "$TARGET_STATUS" '
  ...
  def has_verifiable_conditions:
    ...
    # Numeric thresholds
    (test("\\d+\\s*(%|ms|s|count|times|items|件|個|回)")) or
  ...
  # Field values (null-safe)
  ($p.Description.rich_text // [] | rich_text_content) as $desc |
  ($p["Acceptance Criteria"].rich_text // [] | rich_text_content) as $ac |
  ($p["Execution Plan"].rich_text // [] | rich_text_content) as $plan |
  ($p.Issuer.people // [] | length) as $issuer_count |
  ($p.Assignees.people // [] | length) as $assignees_count |
  ($p.Priority.select.name // null) as $priority |
  ($p.Executor.select.name // null) as $executor |
  ($p["Working Directory"].rich_text // [] | rich_text_content) as $workdir |
  ($p.Branch.rich_text // [] | rich_text_content) as $branch |
  ($p["Agent Output"].rich_text // [] | rich_text_content) as $agent_output |
  ($p["Error Message"].rich_text // [] | rich_text_content) as $error_msg |
  ...
  (if $target == "Ready" or $target == "In Progress" then
    ...
    (if $target == "In Progress" then
      ...
      (if ($executor == "claude-code" or $executor == "cowork") and ($workdir | length) == 0
    ...
    # Warnings
    (if $issuer_count == 0
     then $warnings + [{"field":"Issuer","rule":"recommended","message":"Issuer is empty. Consider running backfill or setting it manually."}]
    ...
    (if $target == "In Progress" and $executor == "claude-code" and ($branch | length) == 0
    ...
    (if ($executor == "claude-code" or $executor == "cowork") and ($agent_output | length) == 0
